...
%PLOTRADIALPOSITIVE2D Render the (radius, z) fraction-positive map as an RGB image.
%   RADPOS2DCOMB = PLOTRADIALPOSITIVE2D(POSITIONS, META, CHANNELTHRESHOLDS,
%   OPTIONS) calls RADIALPOSITIVE2D and visualizes up to three marker channels
%   as the R/G/B planes of a composite (radius, z) image over the negative-cell
%   background. OPTIONS sets channels (1=DAPI), color mode, font size and flip.
%   Returns the computed map RADPOS2DCOMB.
    ...
    for cii = 1:4
        ...
        if ~isnan(ci)
            ...
            summean{ci} = mean(summed_profiles{ci}, 'omitnan');
            sumstd{ci} = std(summed_profiles{ci}, 'omitnan');

% PREPROCESS_IMAGES  Configure and run image preprocessing (MIP/z-stacks/stitch) [template].
%   Sets preprocessing options and metadata for a microscope dataset, then runs
%   the selected steps: make MIPs, write z-stacks, and/or stitch montages (toggle
%   makeMIPs / writeZslices / makeStitchedImages). Edit the metadata and option
%   blocks near the top; run from the raw-data directory.
clear; close all;
...
%images
mipext = '.jpg';
% One value is reused for all grids; pass a cell array with one value per grid
% when different grids were acquired with different snake orders.
...
stitchOptions = struct(... 
    ...
    'mipext', mipext,...
    'snakeorder', snakeorder,... %leftright or bottomtop; sometimes Fusion-generated grids snake left to right, sometimes bottom to top
    ...
    'bigTIFF',false,... % if stitched image exceeds 4GB, set this to true to save as bigtiff
    'exportZarr',false,... % optionally export each stitched stack as OME-Zarr for Python/Dask/napari workflows
    'zarrDir',fullfile(dataDir, 'Zarr'),...
    'zarrOptions',struct('chunks', [1 1 16 512 512]));
...
zSliceOptions = struct('filepattern','',... %look for files automatically if empty, set manually if preferred, e.g. if only running on *FusionStitcher.ims files
    'nucChannel', nucChannel,...
    'writeChannels',  [],... %
    'writeZsubStacks', false,... % if the stack is too big, you can write substacks instead
    'maxZslices', 10,...
    'exportZarr', false,... % optionally export large non-stitched time-series/z-stacks as OME-Zarr
    'zarrDir', fullfile(dataDir, 'Zarr'),...
    'zarrOptions', struct('chunks', [1 1 16 512 512]),...
    'tmax',     []  );

% ANALYZE_LIVE_VS_FIXED  Map live time-lapse cells to fixed endpoint data [template].
%   Builds a LineageTrace linking a live dataset to a fixed dataset, maps cells
%   between them, checks the mapping visually, and plots mean live signaling
%   (CN ratio) per condition. Edit liveDir/fixedDir, channels and conditions.
clear all; close all;
...
for wellnr = 1:nWells
    ...
    for pi = conditionPositions
        ...
        R = mean(lt.live_position(pi).cellData(end).CNratio, 'omitnan');
        eR = std(lt.live_position(pi).cellData(end).CNratio, 'omitnan');

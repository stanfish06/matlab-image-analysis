...
if isfield(opts,'ext') && ~isempty(opts.ext)
    ...
    exts = {'.ims','.nd2','.lif','.vsi'}; %TODO: expand this list
    idx = 1;
    criterion = true;
...
if ischar(snakeorder) || isstring(snakeorder)
    snakeorder = cellstr(snakeorder);
elseif iscell(snakeorder)
    snakeorder = cellfun(@char, snakeorder, 'UniformOutput', false);
else
    error('stitchImageMontages:InvalidSnakeOrder', 'opts.snakeorder must be a char vector, string, or cell array of char vectors/strings.');
end
...
if doFlatFieldCorrection
    if ~(strcmp(ext, '.nd2') || strcmp(ext, '.vsi'))
        doFlatFieldCorrection = false;
...
if isscalar(snakeorder) && ngrids > 1
    snakeorder = repmat(snakeorder, 1, ngrids);
elseif numel(snakeorder) ~= ngrids
    error('stitchImageMontages:SnakeOrderGridMismatch', ...
        'opts.snakeorder must contain either one value or one value per stitched grid.');
end

%determine order of images in a stitched grid
order = repmat({NaN(ppg,2)}, 1, numel(snakeorder));
if ~montageFusionGrid
    for k = 1:numel(snakeorder)
        idx = 1;
        if strcmp(snakeorder{k},'leftright')
            for ii = 1:gridsize(1)
                ...
                order{k}(idx:idx + gridsize(2) - 1,:) = [ii*ones(gridsize(2),1),roworder(:)];
                idx = idx + gridsize(2);
            end
        elseif strcmp(snakeorder{k},'rightleft')
            for ii = 1:gridsize(1)
                if mod(ii,2) == 0 %alternate direction for odd and even rows
                    roworder = 1:gridsize(2);
                else
        ...
        elseif strcmp(snakeorder{k},'leftright_noturn')
            % always goes from left to right, row-by-row
            for ii = 1:gridsize(1)
                roworder = 1:gridsize(2);
                order{k}(idx:idx + gridsize(2) - 1,:) = [ii*ones(gridsize(2),1),roworder(:)];
                idx = idx + gridsize(2);
            end
        elseif strcmp(snakeorder{k},'rightleft_noturn')
            % always goes from left to right, row-by-row
            for ii = 1:gridsize(1)
                roworder = gridsize(2):-1:1;
                order{k}(idx:idx + gridsize(2) - 1,:) = [ii*ones(gridsize(2),1),roworder(:)];
        ...
        elseif strcmp(snakeorder{k},'bottomtop')
            for ii = 1:gridsize(2)
                ...
                order{k}(idx:idx + gridsize(1) - 1,:) = [columnorder(:),ii*ones(gridsize(1),1)];
                idx = idx + gridsize(1);
            end
        end
    end
...
while pidx <= ngrids %TODO: add the option to only do stitching for specified colonies/grids
    ...
    listing = dir(fullfile(dataDir,['*',ext]));
    % sort images if it ends with integer numbers in this format: _#.filetype
    % index_pattern = sprintf("(?<=[F_])(\\d+)(?=\\%s)", ext);
    % [~, sort_idx] = sort(cellfun(@(x)str2double(x), cellfun(@(x)regexp(x, index_pattern, "match"), {listing.name}, 'UniformOutput', false)));
    ...
    if nfiles >= pidx*ppg || seriesflag
        ...
        for ti = 1:nt
            ...
            for idx = 1:ppg
                ...
                ii = order{pidx}(idx,1); jj = order{pidx}(idx,2);
                ...
                for ci = 1:nc
                    for zi = 1:nz
                        if swapZT
                            % temporary fix
                            im = bfGetPlane(r, r.getIndex(ti-1,ci-1,zi-1)+1);
                        else
            ...
            for ci = 1:nc
                ...
                for zi = 1:nz
                    ...
                    switch stitchmode
                        ...
                        otherwise
                            error('stitchImageMontages:UnknownStitchMode', 'Unknown stitchmode: %s', stitchmode);
                    end
                    ...
                    tiffObj = writeStitchedPlane(tiffObj, fname, tmp, bigTIFF, mode, zi, nz);
                end
                %write mip
                [MIP,~] = max(stitched,[],3);
                writeMipImage(MIP, mipname, mipext);

                if exportZarr
                    zarrname = fullfile(zarrDir, ['stitched', suffix(1:end-4), '.zarr']);
                    writeZarrStack(stitched, zarrname, addImageScale(zarrOptions, meta));
                end
        ...
        if nt > 1 && makePreviewVideos
            ...
            if margin > 0
                previewForVideo = preview(1+margin:end-margin, 1+margin:end-margin,:,:);
            else
                previewForVideo = preview;
            end
            for ci = 1:nc
                ...
                timepointavgs = squeeze(mean(previewForVideo(:,:,ci,:),[1,2]));
                [~,I] = sort(timepointavgs,'descend');
                Ilim = stitchedlim(previewForVideo(:,:,ci,I(ceil(nt/50))));
                ...
                open(v)
                for ti = 1:nt
                    writeVideo(v,im2double(imadjust(previewForVideo(:,:,ci,ti),Ilim)))
                end
                close(v);
            end
        end
...
function tiffObj = writeStitchedPlane(tiffObj, fname, imagePlane, bigTIFF, mode, zi, nz)
if bigTIFF
    if zi == 1
        tiffObj = Tiff(fname, 'w8');
    else
        tiffObj.writeDirectory();
    end
    tiffObj.setTag('ImageLength', size(imagePlane,1));
    tiffObj.setTag('ImageWidth', size(imagePlane,2));
    ...
    tiffObj.setTag('RowsPerStrip', size(imagePlane,1));
    tiffObj.write(imagePlane);
    if zi == nz
        tiffObj.close();
        tiffObj = [];
    end
else
    imwrite(imagePlane, fname, 'WriteMode', mode)
end
end

function writeMipImage(MIP, mipname, mipext)
...
function zarrOptions = addImageScale(zarrOptions, meta)
if ~isfield(zarrOptions, 'scale') || isempty(zarrOptions.scale)
    zres = 1;
    hasZres = (isobject(meta) && isprop(meta, 'zres')) || (isstruct(meta) && isfield(meta, 'zres'));
    if hasZres && ~isempty(meta.zres) && ~isnan(meta.zres)
        zres = meta.zres;
    end
    zarrOptions.scale = [1 1 zres meta.yres meta.xres];
end
end

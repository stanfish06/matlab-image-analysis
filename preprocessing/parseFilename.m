...
%PARSEFILENAME Split a microscope filename into base name and position index.
%   [BAREFNAME, PIDX] = PARSEFILENAME(FNAME, DATADIR) parses the raw image
%   filename FNAME, returning the base name BAREFNAME and position index PIDX
%   (empty if none). Handles .ims/.tif/.vsi (trailing 'F####' position
%   suffix) and .nd2/.lif files (single- or multi-series, using Bio-Formats to
%   count series). DATADIR (default: the file's folder) is used to probe for
%   sibling numbered files.

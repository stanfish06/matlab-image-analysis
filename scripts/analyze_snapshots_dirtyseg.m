% ANALYZE_SNAPSHOTS_DIRTYSEG  Snapshot analysis with quick ("dirty") segmentation [template].
%   For plate-layout snapshot data: lists .ims files by well (getPlateFileNames),
%   runs a fast nuclear segmentation, and computes per-condition summaries (e.g.
%   transfection efficiency). Edit the `conditions` table and channel labels.
clear all; close all;

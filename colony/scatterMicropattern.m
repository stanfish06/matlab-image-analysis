...
if isfield(options, 'crossConditionColonySummary')
    crossConditionColonySummary = options.crossConditionColonySummary;
else
    crossConditionColonySummary = false;
end

if isfield(options, 'titleChangeLine') && islogical(options.titleChangeLine)
    titleChangeLine = options.titleChangeLine;
...
if isfield(options, 'savePercent') && islogical(options.savePercent)
    savePercent = options.savePercent;
else
    savePercent = false;
end
...
colonyStats = [];
dat = table('Size', [numel(options.conditionIdx) * 2 * numel(channelCombos) 11], 'VariableType', {'string', 'double', 'string', 'string', 'double', 'double', 'double', 'double', 'double', 'double', 'double'}, 'VariableNames', {'condition', 'condition_idx', 'channel_x', 'channel_y', 'pp', 'pn', 'np', 'nn', 'mean_ncells', 'sd_ncells', 'ncols'});
...
for casei = cases
    for conditionIdx = options.conditionIdx
        ...
        posStartIdx = stats.posStartIdx{conditionIdx};
        nposPerCondition = stats.posPerCondition(conditionIdx);
        ...
        for comboi = 1:numel(channelCombos)
            ...
            if isfield(options, 'colonyAverage')
                colonyAverage = options.colonyAverage;
            else
                colonyAverage = false;
            end

            % mean of log? or should it be the log of mean
            if colonyAverage
                for ci = 1:3
                    colony_x = [];
                    colony_x_se = [];
                    for pidx = 1:nposPerCondition
                        colony_x(end + 1) = mean(x{ci}(posStartIdx(pidx):(posStartIdx(pidx+1)-1)));
                        colony_x_se(end + 1) = std(x{ci}(posStartIdx(pidx):(posStartIdx(pidx+1)-1)));
                    end
                    x{ci} = colony_x;
                    x_se{ci} = colony_x_se;
                    colonyStats(:,channelIdx(ci)) = x{ci}';
                end
            end
            ...
            % Increase point size if plotting colony average
            if isfield(options, 'colonyAverage') && options.colonyAverage
                s = s * options.psEnlarge; % Triple the point size for colony averages
            end


            if isfield(options, 'colonyAverage') && options.colonyAverage
                alpha = 0.4;
                h1 = errorbar(x{1},x{2},x_se{1},"LineStyle","none","Color",[0.5, 0.5, 0.5],"CapSize",0,"LineWidth",1.5);
                hold on
                h2 = errorbar(x{1},x{2},x_se{2},"horizontal","LineStyle","none","Color",[0.5, 0.5, 0.5],"CapSize",0,"LineWidth",1.5);
                ...
                cap_length = 0.05; % Length of the caps
                cap_color = [0.5, 0.5, 0.5, alpha]; % Color of the caps
                for i = 1:length(x{1})
                    line([x{1}(i)-x_se{2}(i), x{1}(i)-x_se{2}(i)], [x{2}(i)-cap_length, x{2}(i)+cap_length], 'Color',cap_color,'LineWidth',1.5);
                    line([x{1}(i)+x_se{2}(i), x{1}(i)+x_se{2}(i)], [x{2}(i)-cap_length, x{2}(i)+cap_length], 'Color',cap_color,'LineWidth',1.5);

                    line([x{1}(i)-cap_length, x{1}(i)+cap_length], [x{2}(i)-x_se{1}(i), x{2}(i)-x_se{1}(i)], 'Color',cap_color,'LineWidth',1.5);
                    line([x{1}(i)-cap_length, x{1}(i)+cap_length], [x{2}(i)+x_se{1}(i), x{2}(i)+x_se{1}(i)], 'Color',cap_color,'LineWidth',1.5);
                end
            end
            ...
            if ~options.minimal
                ...
                if casei == 1
                    clim([options.clim.(collabel)(1) options.clim.(collabel)(2)]);
                end
            ...
            if ~isfield(options,'axisLabel')
                if colonyAverage
                    xlabel([meta.channelLabel{channelIdx(1)} '\_col'])
                    ylabel([meta.channelLabel{channelIdx(2)} '\_col'])
                else
                ...
                if colonyAverage
                    xlabel([options.axisLabel{channelIdx(1)} '\_col'])
                    ylabel([options.axisLabel{channelIdx(2)} '\_col'])
                else
            ...
            if options.minimal
                ...
                xticks(ticklist{1});
                yticks(ticklist{2});
                xtickangle(0); % Disable rotation of x-axis labels
            end
            %ax = ancestor(h, 'axes');
            %ax.XAxis.Exponent = 3;
            %ax.YAxis.Exponent = 3;
            ...
            if ~options.minimal && options.title
                if titleChangeLine
                  title(strrep(meta.conditions{conditionIdx},separator,newline),'FontSize',fs);
                else
                  title(meta.conditions{conditionIdx},'FontSize',fs);
                end
            end
            ...
            if ~options.minimal
                ...
                Ns = stats.ncellsPerCondition{conditionIdx};
                Ncol = stats.posPerCondition(conditionIdx);
                N_mean = round(mean(Ns),3,'significant');
                N_std = round(std(Ns),3,'significant');
                text(xmin(1) + 0.05*(xmax(1)-xmin(1)), xmax(2)*0.9, ['Ncol=' num2str(Ncol)],'FontSize',pfs,'Color',fc,'FontWeight', 'bold','Background','w')
                text(xmin(1) + 0.05*(xmax(1)-xmin(1)), xmax(2)*0.8, ['Ncell=' num2str(N_mean) '(' num2str(N_std) ')'],'FontSize',pfs,'Color',fc,'FontWeight', 'bold','Background','w')
                if savePercent
                    dat(dat_ridx, :) = {meta.conditions{conditionIdx}, conditionIdx, meta.channelLabel{channelIdx(1)}, meta.channelLabel{channelIdx(2)}, pp, pn, np, nn, N_mean, N_std, Ncol};
                    dat_ridx = dat_ridx + 1;
                end
            end
            saveas(gcf, fullfile(dataDir, [meta.channelLabel{channelIdx(1)} '_' meta.channelLabel{channelIdx(2)} '_' strrep(meta.conditions{conditionIdx}, '*', 'star') '_' suffix '.png']));
            close;
        end
    end
end
...
if strcmp(options.imageType,'Disordered')
    results = struct();
    if crossConditionColonySummary
        results.colonyStats = colonyStats;
    end
    if savePercent
        results.percentData = dat;
    end
else
    results = struct();
...
if conditionsCombined
    ...
    for comboi = 1:numel(channelCombos)
        ...
        for i = 1:numel(options.conditionIdx)
            ...
            h = scatter(x, y, c,'filled','MarkerFaceAlpha',ma);
        end
    ...
    for casei = cases
        ...
        for comboi = 1:numel(channelCombos)
            ...
            if casei == 1
                ...
                clim([options.clim(1) options.clim(2)]);
            end
            %title(meta.conditions{conditionIdx});

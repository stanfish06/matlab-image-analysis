% SINGLECELLQUANTIFICATION_STAN  Single-cell quantification driver (Stan's variant) [template].
%   Reads segmentation masks + images from the data folder, builds a Position
%   (disordered) or Colony (micropattern) array with per-cell cellData (nuc/cyt
%   levels, ratios, area, background) and saves positions.mat. Edit the `dirs`,
%   `imageType` and `radii` settings near the top; run from the data directory.
clear; close all; clc

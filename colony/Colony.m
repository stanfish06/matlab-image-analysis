                    ...
                    for i = 1:nBins
                        ...
                        if cyt 
                            cytbindata = this.cellData(ti).cytLevel(idx, cii)...
                                                - this.cellData(ti).background(cii);
                            radialProf.CytAvgSeg(i,cii) = mean(cytbindata, 'omitnan');
                            radialProf.CytStdSeg(i,cii) = std(cytbindata, 'omitnan');
                            
                            R = nucbindata./cytbindata;
                            %R(R < 0.3) = [];
                            %R(R > 2) = [];
                            radialProf.NucCytRatio(i,cii) = mean(R, 'omitnan');
                        end
                        ...
                        radialProf.NucAvgSeg(i,cii) = mean(nucbindata, 'omitnan');
                        radialProf.NucStdSeg(i,cii) = std(nucbindata, 'omitnan');
                        radialProf.nCells(i) = sum(idx);
                        radialProf.NucArea(i) = mean(this.cellData(ti).nucArea(idx), 'omitnan');
                        if cyt
                            radialProf.CytMaskArea(i) = mean(this.cellData(ti).cytMaskArea(idx), 'omitnan');
                        end
                    end
                end

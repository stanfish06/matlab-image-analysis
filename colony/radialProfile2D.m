...
%RADIALPROFILE2D Mean intensity over a (radius, z) grid for one condition.
%   RESULT = RADIALPROFILE2D(STATS, META, CONDI, OPTIONS) bins cells of
%   condition CONDI (from the pooled STATS struct) by radius R and depth Z
%   (scaled to isotropic voxels) into equal-volume bins and returns the mean
%   nuclear intensity per channel on that 2-D grid. OPTIONS sets Rmax/Zmax and
%   half-colony mode.
...
%     ratio_profile_std_tmp = zeros([N-1 nc]);
    ...
    for i = 1:Nzbins-1
        ...
        nuc_profile_z_tmp(i, :) = mean(nucLevel(zidx,:), 'omitnan');
        ...
        for j = 1:Nrbins-1
            ...
            if i == 1
                nuc_profile_r_tmp(j, :) = mean(nucLevel(ridx,:), 'omitnan');
            end
            
            if(sum(combidx)) > 5
                nuc_profile_tmp(i,j,:) = mean(nucLevel(combidx,:), 'omitnan');
            end
        end
    end

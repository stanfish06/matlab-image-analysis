classdef LineageTrace < handle
    % LINEAGETRACE Link live-imaging cell tracks to fixed (endpoint) data.
    %   Associates a live-imaging Position with a fixed-cell Position so that
    %   signaling histories from the time-lapse can be matched to endpoint
    %   fate-marker measurements. Holds both Position objects, their metadata
    %   and image sizes, the live<->fixed index mapping (mapped_idxs), the
    %   relative image shift (shiftyx), and the resulting per-position
    %   histories struct.
    %
    %   Constructors:
    %     LineageTrace(liveDir, fixedDir)            load positions.mat from folders
    %     LineageTrace(posLive, posFixed)            from existing Position objects
    %     LineageTrace(posLive, posFixed, dataDir)
    %     LineageTrace()                             empty object

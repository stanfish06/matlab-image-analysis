function [newedges, badedges, costs] = graphmergelinker(G, points, areas, Divs,...
    max_gap_time)
%Function to resolve merging events in single-cell tracks, since these are
%not genuine biological events, just artifacts of nuclei overlapping in
%some frames but not others
...
mergepoints = find(Din == 2);
splitpoints = find(Dout == 2);
...
for ii = 1:length(mergepoints)
    ...
    if flag
        %if we are linking inputs to merge to outputs of a split, get rid
        %of all intermediary links (links in the merge, links between cells
        %before the split, links to the split) and add two links - one from
        %each input to one output
        ...
        %if not linking inputs to splitting outputs, just discard one of
        %the links
        xyout = allpoints(outs,:);
        Aout = allA(outs);
        c = pdist2(xyin,xyout,'squaredeuclidean').*(1 +...
        ...
        if c(1) < c(2)
            badedges = [badedges; ins(1) outs];
        else
            badedges = [badedges; ins(2) outs];
        end
    end
end

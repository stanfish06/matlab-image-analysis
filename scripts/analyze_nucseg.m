% ANALYZE_NUCSEG  Nuclear-segmentation analysis driver (snapshots) [template].
%   End-to-end template for snapshot data: set up Metadata (manualMeta with
%   plate-layout naming), segment nuclei, quantify per-cell levels, and summarize
%   mean/std cytoplasmic levels per condition. Edit the metadata and segmentation
%   parameters near the top; run from the data directory.
clear all; close all;
...
for ci = 1:meta.nWells
    ...
    cytlevelMean(i,j,:) = mean(cytlevelTot, 'omitnan');
    cytlevelStd(i,j,:) = std(cytlevelTot, 'omitnan');
end
...
for ci = 1:meta.nWells
    ...
    for cpi = 1:meta.posPerCondition
        ...
        cytlevelMean(i,j,cpi,:) = mean(positions(pi).cellData.cytLevel, 'omitnan');
        cytlevelStd(i,j,cpi, :) = std(positions(pi).cellData.cytLevel, 'omitnan');
    end

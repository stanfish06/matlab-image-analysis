...
for si = samples
    ...
    for i = 1:N-1
        ...
        nuc_profile_tmp(i,:) = mean(nLs(I(ptidx),:), 'omitnan');
        nuc_profile_std_tmp(i,:) = std(nLs(I(ptidx),:), 'omitnan');%/sqrt(ptsperbin);
        cyt_profile_tmp(i,:) = mean(cLs(I(ptidx),:), 'omitnan');
        cyt_profile_std_tmp(i,:) = std(cLs(I(ptidx),:), 'omitnan');%/sqrt(ptsperbin);
        ratio_profile_tmp(i,:) = mean(ratios(I(ptidx),:), 'omitnan');
        ratio_profile_std_tmp(i,:) = std(ratios(I(ptidx),:), 'omitnan');%/sqrt(ptsperbin);
    end
...
for i = 1:N-1
    ...
    nuc_profile_tmp(i,:) = mean(nL(I(ptidx),:), 'omitnan');
    nuc_profile_std_tmp(i,:) = std(nL(I(ptidx),:), 'omitnan');%/sqrt(ptsperbin);
    cyt_profile_tmp(i,:) = mean(cL(I(ptidx),:), 'omitnan');
    cyt_profile_std_tmp(i,:) = std(cL(I(ptidx),:), 'omitnan');%/sqrt(ptsperbin);
    %N:C ratio
    ratio_profile_tmp(i,:) = mean(ratio(I(ptidx),:), 'omitnan');
    ratio_profile_std_tmp(i,:) = std(ratio(I(ptidx),:), 'omitnan');
end
...
% between neighbors
nuc_profile_colstd = imfilter(std(cat(3, nuc_profiles{:}),0,3,'omitnan'),[1 1 1 1 1]'/5, 'replicate');
cyt_profile_colstd = imfilter(std(cat(3, cyt_profiles{:}),0,3,'omitnan'),[1 1 1 1 1]'/5, 'replicate');
ratio_profile_colstd = imfilter(std(cat(3, ratio_profiles{:}),0,3,'omitnan'),[1 1 1 1 1]'/5, 'replicate');

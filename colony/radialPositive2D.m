...
%RADIALPOSITIVE2D Fraction of marker-positive cells over a (radius, z) grid.
%   RESULT = RADIALPOSITIVE2D(POSITIONS, META, CHANNELTHRESHOLDS, OPTIONS)
%   pools cells from the Colony array POSITIONS and bins them by radius R
%   (distance from colony center) and depth Z (scaled to isotropic voxels),
%   computing for each channel the fraction of cells whose background-
%   subtracted nuclear level exceeds CHANNELTHRESHOLDS. OPTIONS sets channels,
%   Rmax/Zmax and half-colony mode. RESULT holds the per-channel positive maps.

function [upperleft,links] = registerImageGrid(imgs, pixelOverlap)
    ...
    if isempty(pixelOverlap) 
        logmsg('registerImageGrid: %dx%d grid, no overlap (nominal grid)', size(imgs,1), size(imgs,2));
    ...
    nzlinks = links(links > 0);
    if ~isempty(nzlinks)
        logmsg('registerImageGrid: %dx%d grid, %d overlap links, confidence %.1f-%.1f (mean %.1f)', ...
            size(imgs,1), size(imgs,2), numel(nzlinks), min(nzlinks), max(nzlinks), mean(nzlinks));
    end
